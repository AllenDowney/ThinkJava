...
In Hello.java, the outermost squiggly braces (lines 2 and 8) contain the class definition, and the inner braces (lines 4 and 7) contain the definition of \java{main} method.
...
Line 5 contains a {\bf comment}, or a bit of English text that explains the code that follows.
...
In order to compile Java programs on your own computer, you need to install the Java Development Kit (JDK).
...
Search the web for ``Java SE Downloads'' on Oracle's website.
...
If you run into trouble, there are many detailed tutorials on the web about how to install the JDK for your operating system.

\begin{figure}[h!]
\includegraphics[height=2.4in]{oracle1.png}
\hspace{2em}
\includegraphics[height=2.4in]{oracle2.png}
\caption{Screenshots of Oracle's website (at the time of writing).}
\end{figure}

\index{DrJava}

We will use DrJava (\url{http://drjava.org/}) as the development environment throughout the book.
...
\begin{figure}[h!]
\includegraphics[width=0.9\textwidth]{drjava1.png}
\caption{Download section of DrJava's website (at the time of writing).}
\end{figure}
...
The other settings are for convenience when editing programs over longs periods of time.

\begin{figure}[h!]
\includegraphics[width=0.9\textwidth]{drjava2.png}
\caption{Recommended settings in DrJava preferences.}
\end{figure}
...
The compiler also reports that the problem was found on line 7, which in this case is not at fault.
...
The subsequent lines report the class and method names (Hello.main), along with the file names and line numbers of where the error occurred (Hello.java:6).
