...
% ABD: I don't love this particular piece of mythology, partly because
% it's not accurate, and partly because stories about the old days bore
% students.
...
Low-level languages are only used for programs that need to interact directly with hardware.
...
Later versions of Java are generally backward compatible, so if you are
using a more recent version, the examples in this book should still work.
...
% TODO: when we have the specific URL for the install page, let's
% put it here.
...
the {\bf terminal}.
The command-line is an interface to the operating system, which allows you to run programs, manage files and directories, and monitor system resources.
...
% ABD: There's a conflict here between ``Find the details for your system''
...
Then we run the program using the {\tt java} command, which displays the output on the following line.

Note that the {\tt javac} command requires a source file name, whereas the {\tt java} command requires only the class name.
If you use DrJava, it runs these commands for you; then it collects the output and displays it in the Interactions Pane.
...
People who don't use the command-line interface don't know what they're missing.
...
It adds a special character at the end, called a {\bf newline}, that advances to the beginning of the next line on the screen.
...
You just have to tell Java where to put the newlines.
...
The output is two lines, each ending with a newline:
...
The code \java{\\n} is an example of an {\bf escape sequence}, a sequence
of characters in a string that represents a special character.
