...
def main(name, filename, *argv):
    ...
    ft = Filist(filename)
    ft.move_indexterms()
#    ft.sub_lines(r'\$!\$', r'\\mathit{!}')
#    ft.sub_lines(r'\$([a-zA-Z+-=!()]*)\$', r'\\mathit{\g<1>}')
#    ft.sub_lines(r'\$([^\$]*pi[^\$]*)\$', r'\\( \g<1> \\)')
#    ft.sub_lines(r'\$([^\$]*/[^\$]*)\$', r'\\( \g<1> \\)')
#    ft.sub_lines(r'\$([^\$]*=[^\$]*)\$', r'\\( \g<1> \\)')
#    ft.sub_lines(r'\$([^\$]*O[^\$]*)\$', r'\\( \g<1> \\)')
#    ft.sub_lines(r'\$([^\$]*![^\$]*)\$', r'\\( \g<1> \\)')
#    ft.sub_lines(r'\$([^\$]*\^[^\$]*)\$', r'\\( \g<1> \\)')
#    ft.sub_lines(r'\$([^\$]*\+[^\$]*)\$', r'\\( \g<1> \\)')
    ft.sub_lines(r'\\begin{code}', r'\\begin{verbatim}')
