...
For example \java{count[0]} refers to the ``zeroeth'' element of the array, and \java{count[1]} refers to the ``oneth'' element.  You can use the \java{[]} operator anywhere in an expression:
...
The above \java{for} loop assumes that the array {\tt b} contains at least as many elements as {\tt a}.
...
Each time you invoke {\tt Math.random()} you get the next number in a pseudorandom sequence.
...
To test this method, it is convenient to have a method that prints the contents of an array.
...
In this case, we initialize the array outside the loop; then, inside the loop, we invoke \java{inRange} and store the
result:
