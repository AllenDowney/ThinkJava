...
In this chapter, we present {\bf iteration} using the \java{while} and \java{for} statements.
...
What this code means is: ``While {\tt n} is greater than zero, print the value of {\tt n} and then reduce the value of {\tt n} by 1.
...
If the condition is \java{false} the first time, the statements inside the loop are never executed.
...
As a final example, we can take the code from the last example and
wrap it up in a method:
...
Since we added a new parameter to \java{printMultTable}, we had to change both the method declaration and the line where it is invoked.
...
The \java{for} loop is more concise and easier to read, since it puts all the loop-related statements in one place.  This for loop:
...
Has the same output as this while loop:
...
We can rewrite the final example in the previous section with just a few lines of code:
...
Instead, it uses the variables {\tt row} and {\tt col} to represent positions in the table.
