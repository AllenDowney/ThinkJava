...
In Java and other object-oriented languages, {\bf objects} are data that come with a set of methods.
These methods operate on a specific object, performing computations and sometimes modifying the object's data.
Strings are objects, and their data is a sequence of characters (i.e., letters, numbers, spaces, and symbols).
The {\tt String} class has many methods for manipulating character data, and we will explore some of them in this chapter.
...
The first method we will look at is {\tt charAt}, which allows you to extract characters from a {\tt String}.
...
The code {\tt fruit.charAt(1)} means we invoke the {\tt charAt} method on the object referenced by {\tt fruit}.
...
When you print the value of {\tt letter}, you may be surprised:
...
The 0th (``zeroeth'') letter of {\tt "banana"} is the letter {\tt b}.
...
In source code, character literals appear in single quotes like \java{'c'}.
...
In fact, they are really just 16-bit integers in disguise.
...
Possible characters not only include the letters A to Z and the digits 0 to 9; they also support alphabets like Cyrillic and Greek, and left-to-right scripts like Arabic and Hebrew.
...
This example also demonstrates how you can convert \java{char} and \java{int} values using a cast operator.
...
The name of the loop variable is {\tt i}, which stands for {\bf index} (i.e., the position of each character).
Notice that the condition is {\tt i < fruit.length()}, which means that when {\tt i} is equal to the length of the string, the
condition is \java{false} and the loop terminates.
%The last character we access is the one with the index {\tt fruit.length() - 1}.

\index{String!length}
\index{length!String}

In the above example, {\tt length()} returns the number of characters in the string.
...
This code compiles just fine, but it results in a runtime error.
...
Many string traversal problems involve reading one string and creating another string.
...
The value returned by \java{reverse("banana")} is \java{"ananab"}.
Notice that the {\tt reverse} method begins with \java{""}, the {\bf empty string}.
It then iterates from the last character and down to the zeroth.
...
Other string traversal problems involve looping and counting.
The following method counts the number of times a character appears in a string.
...
The {\tt substring} method returns a copy of the given string, beginning at the specified index.
\begin{itemize}
...
By design, {\tt substring} returns the empty string if the argument is the length of the string.
...
Like many string methods, {\tt substring} is overloaded for convenience.
Another version of {\tt substring} takes two arguments: the start and ``end'' index.
...
Notice that the end index is not included in the string.
...
Designing substring this way allows programmers to specify a length without having to subtract one.
For example, the code could be rewritten as \java{fruit.substring(2, 2 + 3)}.
...
More importantly, it shows the exact line of code where the error occurred.
...
This code finds the index of the letter \java{'a'} in the string.
In this case, the letter appears three times, so it is not obvious what {\tt indexOf} should do.
According to the documentation, it returns the index of the {\em first} appearance.

To find subsequent appearances, there is another version of {\tt indexOf}.
It takes a second argument that indicates where in the string to start looking.
...
The former throws an exception when the index is out of range.
In contrast, {\tt indexOf} returns the value {\tt -1} if the character does not appear in the string.

You can also use {\tt indexOf} to search for an entire string.
For example, the code \java{fruit.indexOf("nan")} returns 2 and \java{fruit.indexOf("zzz")} returns -1.
...
existing string.
Actually, neither these methods nor any others can change a string, because strings are {\bf immutable}.
...
It is often necessary to compare strings to see if they are the same, or to see which comes first in alphabetical order.
...
The syntax here may seem a little awkward.
...
Like the \java{compareTo} method, you too can do useful arithmetic with characters.
For example, \java{letter - 'a'} will tell you what position in the alphabet a lowercase letter appears (keeping in mind that 'a' is the zeroeth letter of the alphabet and 'z' is the 25th).
...
Technically, in both of these examples the typecast \java{(int)} is unnecessary, since Java will convert type \java{char} to type \java{int} automatically.
...
Each of these classes is in the {\tt java.lang} package, so you don't need to \java{import} them.

One nice feature of these classes is they each define the constants \java{MIN_VALUE} and \java{MAX_VALUE}.
...
We can convert integers (and other primitives) to strings using the same technique:
...
\term{Unicode}
The computing industry standard for representing text in most of the world's languages.
...
\term{empty string}
The string \java{""} which exists but has a length of zero.
