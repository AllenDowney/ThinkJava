...
In the remaining chapters, we will develop programs to work with playing cards and decks of cards.
Here is an outline of the road ahead:

\begin{enumerate}

\item In this chapter, we will define a \java{Card} class and write methods that work with \java{Card}s and arrays of \java{Card}s.

\item In Chapter~\ref{chap12}, we will create a \java{Deck} class that encapsulates an array of Cards and write methods that operate on \java{Deck}s.

\item In Chapter~\ref{chap13}, we will introduce object-oriented programming (OOP) and redesign the \java{Card} and \java{Deck} classes accordingly.
...
If you are unfamiliar with common playing cards, now would be a good time to get a deck; otherwise this chapter might not make much sense.
Or read \url{https://en.wikipedia.org/wiki/Standard_52-card_deck}.
...
To create a \java{Card} object that represents the 3 of Clubs, we invoke the \java{new} operator:
...
The output of this code is {\tt Jack of Diamonds}.
...
These operators (\java{<} and \java{>} and the others) don't work for object types.
For \java{String}s, Java provides a \java{compareTo} method.
We have to write our own for \java{Card}s, which we will call \java{compareCard}.
...
Some sets are completely ordered, which means that you can compare any two elements and tell which is bigger.
...
So we simply return zero at the end of the method.
...
They are created when the program starts (when the class is used for the first time) and survive until the program ends.
...
Note that whether a variable is \java{static} and \java{final} are two separate considerations:
...
public class Card {
    ...
    // add instance variables and constructors here
...
Writing \java{static final} variables in ALL CAPS is a common convention that makes it easier to find their definitions.

One advantage of making them class variables is that they don't need to be created (and garbage collected) every time the printCard method is called.
They may also be useful in other methods and classes, so it's nice to have them defined in one place.
...
But if you try to access the instance variables of the non-existent \java{Card}s, you get a \java{NullPointerException}.
...
Note that {\em is} the correct syntax for accessing the \java{rank} of the ``zeroeth'' card in the deck.
Here is another example of composition, or combining the syntax for accessing an element of an array and an instance variable of an object.
...
The next method we'll write is \java{findCard}, which searches an array of \java{Card}s to see whether it contains a certain
card.
...
The arguments of \java{findCard} are \java{cards} and \java{card}.
...
If the cards in the deck are not in order, there is no way to search faster than this.
...
When you look for a word in a dictionary, you don't search linearly through every word, because the words are in alphabetical order.
As a result, you probably use an algorithm similar to a binary search:

\begin{enumerate}

\item Start in the middle somewhere.
\item Choose a word on the page and compare it to the word you are looking for.
\item If you find the word you are looking for, stop.
\item If the word you are looking for comes after the word on the page, flip to somewhere later in the dictionary and go to
step 2.
\item If the word you are looking for comes before the word on the page, flip to somewhere earlier in the dictionary and go to
step 2.
...
Getting back to the deck of cards, if we know the cards are in order, we can write a faster version of \java{findCard}.
One way to write a binary search is with a recursive method, because binary is naturally recursive.

The trick is to write a method that takes two indexes as parameters: \java{low} and \java{high}.
...
\begin{code}
public static int findCard(Card[] cards, Card card, int low, int high) {
    int mid = (high + low) / 2;
...
For example, you might execute the following code:
...
Let's make up a card that is not in the deck, say the ``15 of Diamonds'' \java{new Card(1, 15)}.
If we try to find it using the recursive method, we get the following:
...
So we only have to invoke \java{compareCard} 6 or 7 times, compared to up to 52 times if we did a sequential search.
In general, binary is much faster than a sequential search, and even more so for large arrays.
...
Either error causes infinite recursion, which throws a \java{StackOverflowException}.
...
\begin{exercise}
Encapsulate the deck-building code of Section~\ref{cardarray} in a method called \java{makeDeck} that takes no parameters and returns a fully-populated array of \java{Card}s.
\end{exercise}
