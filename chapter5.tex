...
Conditional operators evaluate the second operand only if necessary.
...
Don't forget that you can try out many of the examples in this book directly in Interactions.
...
We can prevent this error from happening by checking the input before parsing:
...
Note that if you \java{return} from \java{main}, the program ends without running the remaining statements.
...
You should always check the base case first, or else the recursion may keep going until the program runs out of memory.
...
The \java{forever} method prints the string only about 10,000 times before it suddenly terminates with a \java{StackOverflowError}.
As we saw in Section~\ref{debugger}, Java automatically tracks the call and return of each method in memory, much like the stack diagrams we draw on paper.
Each method call takes a little memory, and that memory is freed when the method returns.
