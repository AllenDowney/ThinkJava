...
This chapter presents the features you need for programs like this: {\tt if} statements and a new data type called {\tt boolean}.
...
{\bf Relational operators} are used to check conditions like
whether two values are equal, or whether one is greater than the other.
...
{\tt true} or {\tt false}.  These values have type {\tt boolean};
in fact, they are the only values with type {\tt boolean}.

You are probably familiar with these operations, but notice that the
Java syntax is different from mathematical symbols like $=$, $\neq$
and $\le$.
A common error is to use a single {\tt =} instead of a double {\tt ==}.  Remember that {\tt =} is the assignment operator and {\tt ==} is a comparison operator.
...
{\tt ==} and {\tt !=} work with Strings, but they don't do what you expect.
We'll explain what they do later; in the meantime, don't use them with
Strings.  Instead, you should use the String methods \java{equals} and \java{compareTo}:
...
Conditional operators only evaluate the second operand if they have
to.  For example, \java{true || anything} is always true, so Java does
not need to evaluate the expression \java{anything}.  Likewise,
\java{false && anything} is always false.  Ignoring the second
argument, when possible, is called {\bf short circuit} evaluation, by
analogy with an electrical circuit.  Short circuit evaluation can save
time, especially if \java{anything} is a method call; it can also
avoid unnecessary errors, if \java{anything} might fail.
...
% ABD: I worry about instructions like this, because students seldom follow
% them.  Can we create some exercises (and move this section to the
% end of the chapter)?
...
A second form of decision statement is alternative execution, in which there are two possibilities indicated by \java{if} and \java{else}.  The possibilities are called {\bf branches};
...
If the remainder when {\tt x} is divided by 2 is zero, we know that {\tt x} is even, and this code prints a message to that effect.
...
the first branch contains a \java{print} statement; the second branch contains another decision statement, which has
two branches of its own.  Each of them is a \java{print} statement, but they could have been decision statements as well.
...
%Although Java does not require braces for one-line if statements,  we %recommend them...

% ABD: I don't like to give style advice unless I explain why.  Otherwise
% it's just religion.
...
This example uses \java{System.err}, which is the {\it standard error} output stream.
...
% ABD: I don't agree with this advice.  I think there's nothing wrong
% with methods that peel off special cases one at a time, with one
% return statement per special case.
...
The total number of newlines is \java{1 + (n - 1)}, which is just what we wanted: \java{n}.
...
It's easier to draw stack diagrams from top to bottom, because you don't know how much vertical space you will need.
...
% ABD: we have not defined loop yet
%This type of recursion is essentially a simple loop.

% ABD: I prefer to leave out this kind of forward reference
...
% ABD: I love this example.  Nice addition!
...
% TODO: we should have an example that demonstrates infinite recursion
...
When the recursive call returns, the method prints one digit of the result and returns.
